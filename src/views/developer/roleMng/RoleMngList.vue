        ...
        <div class="box_section">
          <div class="search_box col_3">
            <ul>
              <li>
                <p>Mã Role</p>
                <InputBase v-model:modelValue="dataSearch.roleCd" id="roleCd" />
              </li>
              <li>
                <p>Tên Role</p>
                <InputBase v-model:modelValue="dataSearch.roleNm" id="roleNm" />
              </li>
              <li>
                <p>Loại Site</p>
                <SelectBoxBase
...
import { getPageData } from "@/stores/developer/roleMng/roleMng.service";
import { AdRoleFilterRequest, AdRoleResDTO } from "@/stores/developer/roleMng/roleMng.type";
  ...
  const dataSearch = ref<AdRoleFilterRequest>({
    roleCd: "",
    roleNm: "",
    roleSiteCd: "",
    page: 1,
  ...
  onBeforeMount(async () => {
    ...
    await getListCodeMng({ upCdIdList: [UP_CD_SITE] }).then(
      (res) => {
  ...
  function goAction(data: AdRoleResDTO) {
    router.push({
      name: SCREEN.roleManagementForm.name,
      state: { id: data.roleId },
    });
  }
  ...
  const fnPagination = async (pageNumber: number, pagesSize: number) => {
    ...
    await getPageData(dataSearch.value).then((res) => {
      ...
      data.value = res.data.data.data.map((item: AdRoleResDTO, index: number) => {
        item.rowNum =
